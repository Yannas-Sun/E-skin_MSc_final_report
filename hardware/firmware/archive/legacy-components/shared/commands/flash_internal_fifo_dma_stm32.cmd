...
if "%~1"=="" (set "GAP_US=37") else (set "GAP_US=%~1")
if "%~2"=="" (set "MUX_US=0") else (set "MUX_US=%~2")
...
set "BUILD=%ROOT%\.cmake-build\INTERNAL_FIFO_DMA_GAP_%GAP_US%_MUX_%MUX_US%"
...
echo ============================================================
echo MAX11633 internal-clock FIFO-DMA experiment
echo Command-to-read gap: %GAP_US% us   MUX settle: %MUX_US% us
echo Stable Combined firmware is not modified.
echo ============================================================
...
cmake --fresh -S "%SOURCE%" -B "%BUILD%" -G Ninja ^
  ...
  -DSCLK33_PROFILE=internal_fifo_dma ^
  -DINTERNAL_FIFO_GAP_US=%GAP_US% ^
  -DINTERNAL_FIFO_MUX_SETTLE_US=%MUX_US% ^
  -DSCLK33_FSR_HZ=0 ^
  -DSCLK33_ACC_HZ=1 ^
