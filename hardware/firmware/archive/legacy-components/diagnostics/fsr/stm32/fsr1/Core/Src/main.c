...
/* High-rate standalone-test setting. This is intentionally more aggressive
 * than the combined application's validated 100 us and must be checked for
 * pressure response and adjacent-row crosstalk on the physical FSR1. */
#define FSR_MUX_SETTLE_US     25U
#define HOST_FRAME_MAGIC     0xA5U
#define HOST_FRAME_BYTES     (1U + 2U + (FSR_ROWS * FSR_COLUMNS * 2U))
#define HOST_SPI_TIMEOUT     10U
/*
...
static volatile uint8_t host_dma_complete = 0U;
static volatile uint8_t host_dma_error = 0U;
...
static HAL_StatusTypeDef MAX11633_StartScan(void);
static HAL_StatusTypeDef MAX11633_WaitEoc(void);
static HAL_StatusTypeDef MAX11633_ReadResults(uint16_t *samples);
...
static void delay_until_us(uint32_t started, uint32_t microseconds)
{
  ...
  while ((DWT->CYCCNT - started) < cycles)
  {
    __NOP();
  }
}
...
static void MX_SPI1_Init(void)
{
  ...
  /* 80 MHz APB2 / 8 = 10 MHz SPI data clock. ADC conversion remains on its
   * internal clock (CKSEL=10); this is not a 10 MHz external conversion. */
...
/**
  * Enable DMA/DMAMUX and the two channels used by full-duplex SPI3.
  */
static void MX_DMA_Init(void)
{
...
static HAL_StatusTypeDef MAX11633_StartScan(void)
{
  ...
  HAL_StatusTypeDef status = HAL_SPI_Transmit(
      &hspi1, &conversion_command, 1U, MAX11633_EOC_TIMEOUT);
  HAL_GPIO_WritePin(GPIOB, ADC_CS1_Pin, GPIO_PIN_SET);
  return status;
}
...
static HAL_StatusTypeDef FSR_ScanFrame(uint16_t frame[FSR_ROWS][FSR_COLUMNS])
{
  ...
  for (uint8_t row = 0U; row < FSR_ROWS; ++row)
  {
    ...
    HAL_StatusTypeDef status = MAX11633_StartScan();
    if (status == HAL_OK)
    {
      status = MAX11633_WaitEoc();
    }

    /* Conversion results are now held inside the ADC. Start the next MUX
     * address settling before clocking out the current 32-byte result FIFO. */
    HAL_GPIO_WritePin(GPIOA, MUX_EN1_Pin, GPIO_PIN_SET);
    if ((status == HAL_OK) && ((row + 1U) < FSR_ROWS))
    {
      MUX1_SelectRow((uint8_t)(row + 1U));
      HAL_GPIO_WritePin(GPIOA, MUX_EN1_Pin, GPIO_PIN_RESET);
      mux_settle_started = DWT->CYCCNT;
    }
    ...
    if (status == HAL_OK)
    {
      status = MAX11633_ReadResults(raw_samples);
    }
    if (status != HAL_OK)
    {
      HAL_GPIO_WritePin(GPIOA, MUX_EN1_Pin, GPIO_PIN_SET);
...
static HAL_StatusTypeDef HOST_SendFrame(const uint16_t frame[FSR_ROWS][FSR_COLUMNS])
{
...
#else
  /* STM32G4 is little-endian and raw samples were already masked to 12 bits. */
  memcpy(&host_tx_frame[offset], frame, sizeof(fsr_frame));
  offset += sizeof(fsr_frame);
#endif

  /* Reset stale slave state, then arm both DMA directions before HOST_IRQ.
   * RX DMA is essential at 10 MHz: blocking HAL polling can overrun the SPI3 RX
   * FIFO and then wait for the full one-second timeout after a valid TX frame. */
  ...
  host_dma_error = 0U;
  status = HAL_SPI_TransmitReceive_DMA(&hspi3, host_tx_frame, host_rx_dummy,
                                       HOST_FRAME_BYTES);
  if (status != HAL_OK)
  {
    return status;
  ...
  while ((host_dma_complete == 0U) && (host_dma_error == 0U) &&
         ((HAL_GetTick() - started) < HOST_SPI_TIMEOUT))
  {
    __WFI();
  }
  ...
  (void)HAL_SPI_Abort(&hspi3);
  return (host_dma_error != 0U) ? HAL_ERROR : HAL_TIMEOUT;
}
