...
/* Match the GUI calibrated-display curve: normalized load = linear_load^4. */
#define FSR_NORMALIZATION_GAMMA_FOURTH 1U
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  /* The isolated SCLK33 experiment can double the CPU clock while the stable
   * combined application remains at its established 80 MHz setting. */
  RCC_OscInitStruct.PLL.PLLState = RCC_PLL_ON;
  RCC_OscInitStruct.PLL.PLLSource = RCC_PLLSOURCE_HSI;
  RCC_OscInitStruct.PLL.PLLM = RCC_PLLM_DIV4;
#ifdef SCLK33_SYSCLK_HIGH
  /* Isolated sweep: 16 MHz HSI / 4 * PLLN / 2. */
  RCC_OscInitStruct.PLL.PLLN = SCLK33_PLLN;
#else
...
static void MX_SPI1_Init(void)
{
  ...
  /* MAX11633 data clock: 80 MHz / 8 normally, 160 MHz / 16 in the isolated
   * SCLK33 high-clock experiment. Both produce exactly 10 MHz. */
...
static void MX_SPI2_Init(void)
{
  ...
  /* Preserve a 10 MHz ACC clock at both supported CPU-clock profiles. */
#ifdef SCLK33_SYSCLK_HIGH
                                    ...
                                    uint16_t raw_sample)
{
  ...
  /* Round to the nearest linear 12-bit code instead of truncating downward. */
  normalized = ((numerator * 0x0FFF) + (denominator / 2)) / denominator;
#if FSR_NORMALIZATION_GAMMA_FOURTH
  /* Apply x^4: strongly compress low loads and emphasize high loads. */
  normalized = ((normalized * normalized) + (0x0FFF / 2)) / 0x0FFF;
  normalized = ((normalized * normalized) + (0x0FFF / 2)) / 0x0FFF;
#endif
  if (normalized < 0)
  {
