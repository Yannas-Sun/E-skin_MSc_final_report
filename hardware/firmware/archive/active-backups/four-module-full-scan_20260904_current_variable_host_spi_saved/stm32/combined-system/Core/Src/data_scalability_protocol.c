...
#define DELTA_MASK_TOTAL_BYTES (2U * SPATIAL_MASK_BYTES)
...
#define SPATIAL_PREFIX_BYTES 46U
...
#define SPATIAL_MAX_SAMPLED ((DATA_SCALABILITY_FRAME_BYTES - SPATIAL_PREFIX_BYTES) / 4U)
...
static void mask_set(uint8_t mask[SPATIAL_MASK_BYTES], uint8_t row,
                     uint8_t column);
static uint8_t mask_get(const uint8_t mask[SPATIAL_MASK_BYTES],
                        uint16_t position);
                               ...
                               uint32_t now_ms)
{
  ...
  uint8_t changed_mask[2][SPATIAL_MASK_BYTES];
  ...
  /* A mask Delta frame must fit the maximum Host SPI frame capacity. If all
   * or nearly all cells change, send a complete frame instead of truncating
   * the values. */
  ...
  memcpy(&target[offset], changed_mask[0], SPATIAL_MASK_BYTES);
  offset += SPATIAL_MASK_BYTES;
  memcpy(&target[offset], changed_mask[1], SPATIAL_MASK_BYTES);
  offset += SPATIAL_MASK_BYTES;
  for (uint8_t layer = 0U; layer < 2U; ++layer)
  {
    const uint16_t *current = (layer == 0U) ? fsr1 : fsr2;
                             ...
                             uint32_t now_ms)
{
  ...
  if (sampled > SPATIAL_MAX_SAMPLED)
  {
    /* A full 256-cell ESPF is 26 bytes larger than the FSR-only Host slot.
     * Preserve the same physical data by using the fixed ESKF fallback. */
    const uint16_t length = pack_esk_full(
        target, fsr1, fsr2,
        (uint8_t)(base_flags | FLAG_SPATIAL_FRAME), now_ms);
  ...
  if (mask_changed != 0U)
  {
    ...
    write_spatial_prefix(target, MAGIC_ESPF, now_ms, next_mask, sampled);
  ...
  {
    /* Resynchronise instead of truncating a variable spatial update. */
    length = (uint16_t)(SPATIAL_PREFIX_BYTES + 4U * sampled);
    write_spatial_prefix(target, MAGIC_ESPF, now_ms, next_mask, sampled);
    uint32_t offset = SPATIAL_PREFIX_BYTES;
    for (uint16_t position = 0U; position < FSR_CELLS; ++position)
  ...
  else
  {
    ...
    write_spatial_prefix(target, MAGIC_ESPD, now_ms, next_mask, changed);
    uint32_t offset = SPATIAL_PREFIX_BYTES;
    for (uint8_t layer = 0U; layer < 2U; ++layer)
    {
      const uint16_t *current = (layer == 0U) ? fsr1 : fsr2;
      const uint16_t *previous =
          (layer == 0U) ? previous_fsr1 : previous_fsr2;
      for (uint8_t row = 0U; row < 16U; ++row)
      {
        for (uint8_t column = 0U; column < 16U; ++column)
        {
          ...
          target[offset++] = (uint8_t)((layer << 7U) | row);
...
void DataScalability_ApplyCommand(const uint8_t *command, uint16_t length)
{
  if ((command == NULL) || (length < DATA_SCALABILITY_COMMAND_BYTES) ||
      (memcmp(command, COMMAND_MAGIC, 4U) != 0) ||
      (command[4] != DATA_SCALABILITY_COMMAND_VERSION))
  {
    return;
  }
  const uint8_t requested_mode =
      (uint8_t)(command[5] & DATA_SCALABILITY_COMMAND_MODE_MASK);
  DataScalability_SetMode(requested_mode);
  if ((command[5] & DATA_SCALABILITY_COMMAND_FLAG_DELTA_RESYNC) != 0U &&
  ...
  if ((command[5] & DATA_SCALABILITY_COMMAND_FLAG_SPATIAL_RESYNC) != 0U &&
      mode == DATA_SCALABILITY_MODE_SPATIAL)
  {
    /* The host may have missed a Spatial delta frame. Force the next encoded
     * frame to contain the complete active software mask and sampled values.
     * Keep the zero-load baseline unchanged during transport recovery. */
    spatial_cache_valid = 0U;
  }
                                ...
                                uint8_t base_flags)
{
  ...
  {
    return pack_spatial(target, fsr1, fsr2, base_flags, now_ms);
  }
