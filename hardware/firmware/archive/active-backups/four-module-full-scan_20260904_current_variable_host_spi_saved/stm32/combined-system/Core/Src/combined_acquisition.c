...
static uint16_t host_tx_length[2];
...
static uint8_t host_command_is_valid(const uint8_t *command)
{
  return (uint8_t)(command[0] == 'D' && command[1] == 'S' &&
                   ...
                   (command[5] & DATA_SCALABILITY_COMMAND_MODE_MASK) <=
                       DATA_SCALABILITY_MODE_SPATIAL);
}
...
{
  const uint16_t frame_length = host_tx_length[buffer_index];
  if ((frame_length == 0U) || (frame_length > HOST_FRAME_BYTES))
  {
  ...
  HAL_StatusTypeDef status = HAL_SPI_TransmitReceive_DMA(
      &hspi3, host_tx[buffer_index], host_rx[buffer_index], frame_length);
  if (status != HAL_OK)
  {
    ++host_dma_error_count;
    return status;
...
void CombinedAcquisition_RunOnce(void)
{
  ...
  {
    host_tx_length[host_send_index] =
  ...
  /* Acquire the next frame while the previous valid frame is sent. */
  host_tx_length[host_fill_index] =
  ...
  if (status == HAL_OK)
  {
    ...
    const uint8_t command_flags = command_valid ?
        (uint8_t)(host_rx[released_index][5] &
                  (DATA_SCALABILITY_COMMAND_FLAG_DELTA_RESYNC |
                   DATA_SCALABILITY_COMMAND_FLAG_SPATIAL_RESYNC)) : 0U;
        ...
        (spatial_resync_requested != 0U))
    {
      ...
      host_tx_length[host_send_index] = DataScalability_Encode(
