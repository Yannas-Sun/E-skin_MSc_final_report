 ...
 * two 16 x 16 FSR layers per module, and variable-length Host SPI frames.
...
constexpr size_t HOST_FRAME_CAPACITY_BYTES = 1044;
constexpr size_t HOST_COMMAND_BYTES = 12;
...
constexpr size_t SPATIAL_PREFIX_BYTES = 46;
constexpr uint8_t COMMAND_VERSION = 3U;
constexpr size_t MAX_PACKET_BYTES = PACKET_HEADER_BYTES +
                                     MODULE_COUNT * (4 + HOST_FRAME_CAPACITY_BYTES) +
...
constexpr uint8_t COMMAND_FLAG_DELTA_RESYNC = 0x40U;
constexpr uint8_t COMMAND_FLAG_SPATIAL_RESYNC = 0x80U;
...
uint8_t moduleFrames[MODULE_COUNT][HOST_FRAME_CAPACITY_BYTES] = {};
...
uint16_t logicalFrameLength(const uint8_t *frame) {
  if (markerIs(frame, ESK_FULL)) {
    if (frame[4] != ESK_PROTOCOL_VERSION ||
        readU16LE(&frame[6]) != HOST_FRAME_CAPACITY_BYTES) return 0;
    return HOST_FRAME_CAPACITY_BYTES;
  }
  if (markerIs(frame, ESK_DELTA)) {
    ...
    const uint16_t expected = (uint16_t)(DELTA_MASK_MIN_FRAME_BYTES +
                                         2U * changed);
    if ((declared != expected) || declared > HOST_FRAME_CAPACITY_BYTES ||
  ...
  if (markerIs(frame, ESP_FULL) || markerIs(frame, ESP_DELTA)) {
    ...
    const uint16_t declared =
        (uint16_t)(SPATIAL_PREFIX_BYTES + 4U * count);
    return declared <= HOST_FRAME_CAPACITY_BYTES ? declared : 0;
  }
  if (markerIs(frame, ESP_HEARTBEAT)) return 12U;
  return 0;
}

size_t hostMetadataLength(const uint8_t *frame) {
  if (markerIs(frame, ESK_FULL) || markerIs(frame, ESP_HEARTBEAT)) {
    return HOST_COMMAND_BYTES;
  }
  if (markerIs(frame, ESK_DELTA)) return DELTA_MASK_PREFIX_BYTES;
  if (markerIs(frame, ESP_FULL) || markerIs(frame, ESP_DELTA)) {
    return SPATIAL_PREFIX_BYTES;
  }
  return 0U;
}
...
ReadResult readModuleFrame(uint8_t module) {
  ...
  memset(frame, 0, HOST_FRAME_CAPACITY_BYTES);
  ...
  /* The first 12 bytes carry the command on MOSI and the beginning of the
   * data frame on MISO. Continue only through the metadata needed to learn
   * the declared length, then clock exactly the remaining data bytes. */
  SPI.transfer(frame, HOST_COMMAND_BYTES);
  const size_t metadataLength = hostMetadataLength(frame);
  uint16_t length = 0U;
  if (metadataLength >= HOST_COMMAND_BYTES) {
    if (metadataLength > HOST_COMMAND_BYTES) {
      SPI.transfer(&frame[HOST_COMMAND_BYTES],
                   metadataLength - HOST_COMMAND_BYTES);
    }
    length = logicalFrameLength(frame);
    if ((length != 0U) && (length >= metadataLength)) {
      if (length > metadataLength) {
        SPI.transfer(&frame[metadataLength], length - metadataLength);
      }
    } else {
      length = 0U;
    }
...
void fillCommand(uint8_t command[HOST_COMMAND_BYTES],
  ...
  if (requestDeltaResync) {
    command[5] |= COMMAND_FLAG_DELTA_RESYNC;
  }
  if (requestSpatialResync) {
    command[5] |= COMMAND_FLAG_SPATIAL_RESYNC;
  }
}
