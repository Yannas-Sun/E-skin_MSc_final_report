...
/* Each length belongs to its encoded TX buffer and changes only while that
 * buffer is not owned by the host DMA. Buffer capacity remains 1044 bytes. */
static uint16_t host_frame_length[2];
...
{
  const uint16_t length = host_frame_length[buffer_index];
  if ((length < HOST_COMMAND_BYTES + 4U) || (length > HOST_FRAME_BYTES))
  {
  ...
  HAL_StatusTypeDef status = HAL_SPI_TransmitReceive_DMA(
      &hspi3, host_tx[buffer_index], host_rx[buffer_index], length);
  if (status != HAL_OK)
  {
    ++host_dma_error_count;
    return status;
...
static HAL_StatusTypeDef wait_frame_dma(uint16_t length)
{
  const uint32_t started = HAL_GetTick();
  while ((host_dma_complete == 0U) && (host_dma_error == 0U) &&
         ((HAL_GetTick() - started) < HOST_TIMEOUT_MS))
  {
    /* The master may reject the header and release NSS after only 16 bytes.
     * No clocks yet (remaining == length) is an ordinary pending read, and
     * a pause between header and payload is valid while NSS remains low. */
    if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_15) == GPIO_PIN_SET)
    {
      __DMB();
      const uint32_t remaining = __HAL_DMA_GET_COUNTER(hspi3.hdmarx);
      if ((remaining > 0U) && (remaining < length) &&
          (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_15) == GPIO_PIN_SET) &&
          (host_dma_complete == 0U) && (host_dma_error == 0U))
      {
        host_dma_error = 1U;
        ++host_dma_error_count;
        break;
...
static uint16_t acquire_and_encode(uint8_t *target)
{
  wait_for_requested_scan_period();
  const uint8_t status = scan_fsr_rows();
  last_scan_status = status;
  return DataScalability_Encode(
...
void CombinedAcquisition_RunOnce(void)
{
  ...
  {
    host_frame_length[host_send_index] =
        acquire_and_encode(host_tx[host_send_index]);
  ...
  /* Acquire the next full FSR scan into the other buffer while one complete
   * encoded frame is sent by a single DMA, even if the master pauses clocks
   * after the header with NSS held low. */
  host_frame_length[host_fill_index] =
      acquire_and_encode(host_tx[host_fill_index]);

  const HAL_StatusTypeDef status =
      wait_frame_dma(host_frame_length[host_send_index]);
  ...
  if (status == HAL_OK)
  {
        ...
        (delta_resync_requested != 0U))
    {
       ...
       * this step, one stale Delta frame could be sent before the requested
       * base frame. This buffer is not DMA-owned after wait_frame_dma. */
      host_frame_length[host_send_index] = DataScalability_Encode(
