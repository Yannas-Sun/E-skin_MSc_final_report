/*
 * Variable-length four-module FSR-only bridge for Teensy 4.1.
 *
 * Wire contract: ESK protocol v4 from STM32, MUL1 protocol v2 to the PC,
 * two 16 x 16 FSR layers per module. Host SPI clocks only the encoded frame:
 * 16 header bytes, then length - 16 bytes under the same chip select.
 */
#include <Arduino.h>
#include <SPI.h>
...
constexpr size_t HOST_MAX_FRAME_BYTES = 1044;
constexpr size_t HOST_HEADER_BYTES = 16;
...
constexpr size_t MAX_PACKET_BYTES = PACKET_HEADER_BYTES +
                                     MODULE_COUNT * (4 + HOST_MAX_FRAME_BYTES) +
 ...
 * the packet that contained the Delta base frame. */
...
enum class Algorithm : uint8_t { Full = 0, Delta = 1 };
...
uint8_t moduleFrames[MODULE_COUNT][HOST_MAX_FRAME_BYTES] = {};
...
void fillCommand(uint8_t command[HOST_COMMAND_BYTES], bool requestDeltaResync);
...
/* Only inspect the bytes already clocked from the slave. A valid header
 * bounds the second transfer; the mask and CRC are checked after it ends. */
uint16_t headerFrameLength(const uint8_t *frame) {
  if (frame[4] != ESK_PROTOCOL_VERSION ||
      (frame[5] & 0x10U) == 0U || (frame[5] & 0xC0U) != 0U) return 0U;
  const uint16_t declared = readU16LE(&frame[6]);
  if (markerIs(frame, ESK_FULL)) {
    return declared == HOST_MAX_FRAME_BYTES ? declared : 0U;
  }
  if (markerIs(frame, ESK_DELTA)) {
    if ((frame[5] & 0x20U) == 0U ||
        declared < DELTA_MASK_MIN_FRAME_BYTES ||
        declared > HOST_MAX_FRAME_BYTES || (declared & 1U) != 0U) return 0U;
    return declared;
  }
  return 0U;
}
...
uint16_t logicalFrameLength(const uint8_t *frame) {
  const uint16_t declared = headerFrameLength(frame);
  if (declared == 0U) return 0U;
  if (markerIs(frame, ESK_FULL)) {
    return declared;
  }
  if (markerIs(frame, ESK_DELTA)) {
...
ReadResult readModuleFrame(uint8_t module) {
  ...
  fillCommand(command, requestDeltaResync);
  memset(frame, 0, HOST_MAX_FRAME_BYTES);
  ...
  SPI.transfer(frame, HOST_HEADER_BYTES);
  const uint16_t wireLength = headerFrameLength(frame);
  if (wireLength != 0U) {
    /* Keep CS asserted: STM32 already armed one DMA for the full frame.
     * The 16-byte MOSI command was sent during the header transfer. */
    SPI.transfer(frame + HOST_HEADER_BYTES, wireLength - HOST_HEADER_BYTES);
  }
  /* An invalid header ends after 16 bytes. The slave detects the partial
   * transaction when NSS is released and aborts its outstanding DMA. */
  delayMicroseconds(CS_HOLD_US);
  digitalWrite(CS_PINS[module], HIGH);
  SPI.endTransaction();

  const uint16_t length = wireLength != 0U ? logicalFrameLength(frame) : 0U;
  if (length == 0U) {
...
void fillCommand(uint8_t command[HOST_COMMAND_BYTES],
  ...
  /* Bytes 8..9 are reserved zero; scan rate retains its DSCM v2 offset. */
...
void pollModules() {
  ...
  for (uint8_t module = 0; module < MODULE_COUNT; ++module) {
    ...
    if ((activeModuleMask & moduleBit) == 0U) {
      /* A reconnected module rejoins immediately. Delta needs a
