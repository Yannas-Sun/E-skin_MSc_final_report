"""Seven report 2.4 figures from the provenance-checked compiled Power matrix.

All 15 physical combinations appear once for each recorded ZERO/MAX label.
Two entries combine retained readings with targeted module retests; no new
whole-configuration repeats are implied.
...
INPUT = ROOT / "DATA" / "reference" / "report_v2_4_analysis"
OUTPUT = ROOT / "DATA" / "reference" / "report_v2_4_figures"
...
def plot_scaling(runs, out):
    ...
    for ax, field, ylabel, tag in (
        ...
        for load in LOADS:
            ...
            for n, x in zip(range(1, 5), xx):
                ...
                for row, point_x, point_y in zip(group, x + spread, yy):
                    if ((load == "ZERO" and row["combo"] == "M0-M1-M2")
                            or (load == "MAX" and row["combo"] == "M1-M2-M3")):
                        ax.scatter(point_x, point_y, color=COLORS[load], marker="*",
                                   s=90, edgecolors=DARK_CHARCOAL, linewidths=.5, zorder=7)
    ...
    legend(fig, load_handles() + [Line2D([], [], color=DARK_CHARCOAL, ls="--", marker="D",
                                       mfc="white", ms=4.3, lw=.9, label="Mean ± combination SD"),
                                        Line2D([], [], marker="*", color=DARK_CHARCOAL, ls="none", ms=8,
                                               label="Entry with targeted current retest")], columns=2, y=.98)
    return export(fig, out, "current_power_scaling",
                  "Thirty compiled configuration/load entries, incorporating two targeted module retests: current sums and cross-record source-output power estimates, with combination SD.",
                  {"points": "All 15 module combinations per load; one compiled entry per combination and load. Two entries combine targeted current retests with retained branch readings.",
                   "means": "Arithmetic means by load and N, counts 4, 6, 4, 1. All individual configuration points shown; stars overlay the two N3 compiled entries including targeted current retests.",
                   "uncertainty": "Sample SD (ddof=1) across different combinations for N=1,2,3, never same-condition repeat SD. N=4 SD absent.",
                   "power": "Retained source main voltage multiplied by compiled sequential branch-current sum; two entries include targeted current retests. Excludes Teensy USB and regulator loss.",
...
def plot_voltage(runs, branches, event, out):
            ...
            for row, offset in zip(group, offsets):
                ...
                for branch, jj in zip(bb, local):
                    ...
                    corrected = ((load == "ZERO" and row["combo"] == "M0-M1-M2" and branch["module_id"] == "M0")
                                 or (load == "MAX" and row["combo"] == "M1-M2-M3" and branch["module_id"] == "M3"))
    ...
    handles = [Line2D([], [], marker="^", color=SLATE_BLUE, ls="none", label="Source main"),
               ...
               Line2D([], [], marker="x", color=DARK_CHARCOAL, ls="none", label="Earlier reading (see notes)")]
    legend(fig, handles, columns=3, y=.98)
    return export(fig, out, "voltage_scaling",
                  "Thirty retained source readings and 64 selected module voltage readings, with M0 and M3 targeted voltage retests and two separate earlier readings.",
                  {"points": "30 retained source main and 64 selected module main readings; two additional historical markers: M0 confirmed connector fault and M3 earlier reading of unresolved cause.",
                   ...
                   "correction": "Stars: M0=3.241 V from an earlier separate voltage retest; M3=3.243 V from targeted MAX retest after connector reinsertion. Sources remain user-confirmed 3.276 V. M0 voltage extrema remain from its earlier voltage retest. No extra independent full run.",
...
def plot_drop(runs, branches, out):
        ...
        for iy, (_, combo) in enumerate(combos):
            for ix in range(4):
                ...
                if load == "ZERO" and combo == "M0-M1-M2" and ix == 0:
                    label += "*"
                if load == "MAX" and combo == "M1-M2-M3" and ix == 3:
                    label += "†"
    ...
    fig.text(.225, .025, "* M0 earlier voltage retest; † M3 targeted retest. Sources retained (3.276 V).", fontsize=7)
    return export(fig, out, "module_voltage_drop",
                  "Main-reading voltage differences for all 15 combinations and both recorded load labels; marked M0 and M3 pointwise retest estimates.",
                  {"cells": "64 active-module cells: 1000 × (source main V − accepted module main V), sequential observations.",
                   "correction": "35* mV in ZERO M0-M1-M2/M0 uses earlier retested M0=3.241 V; 33† mV in MAX M1-M2-M3/M3 uses targeted retested M3=3.243 V. Both sources retained at user-confirmed 3.276 V. Cross-record estimates, not synchronous new measurements. Earlier differences 52 mV (M0 confirmed fault) and 44 mV (M3 cause unconfirmed) remain in source records and the report retest table.",
                   ...
                   "uncertainty": "One selected value per active configuration/load/module cell; targeted retests do not create additional full-configuration replicates."})
...
def plot_prediction(predictions, out):
    ...
    for load in LOADS:
        ...
        right.scatter([number(p, "residual_percent_of_prediction") for p in rows], yy,
                      ...
                      edgecolors=DARK_CHARCOAL, linewidths=.4, zorder=4)
    left.set(xlim=(lower, upper), ylim=(lower, upper), xlabel="Separate N=1 current sum (mA)", ylabel="Compiled multi-module sum (mA)")
    ...
    return export(fig, out, "independent_current_prediction",
                  ...
                  {"prediction": "Sum of separate N=1 main currents of exactly the physical modules present, using the same ZERO/MAX label.",
                   "points": "11 distinct multi-module combinations per load, 22 comparisons total; one compiled entry per combination/load, including two entries with targeted current retests. No independent full-condition repeats.",
...
def main():
    ...
    with plt.style.context(STYLE), matplotlib.rc_context(PUBLICATION_STYLE):
        matplotlib.rcParams.update({"svg.hashsalt": "eskin-power-report-2.4", "font.size": 8.5,
    ...
    for entry in figures:
        if entry["stem"] == "branch_current_balance":
            entry["semantics"].update(points="All 64 selected branch main readings, eight combination/load entries per physical module per load; two selected currents come from targeted retests.",
    ...
    manifest = {
        "report_version": "2.4", "audience": "Imperial individual-project manuscript; no publisher-compliance claim",
        "data_counts": {"configuration_conditions": 30, "branches": 64, "combinations_per_load_by_N": [4, 6, 4, 1],
                        "compiled_entries_per_configuration_load": 1, "targeted_module_retest_sets": 2, "additional_full_configuration_repeats": 0, "historical_blocked_screening_only": 4},
    ...
    if args.copy_to_report:
        ...
        shutil.copy2(manifest_path, REPORT_FIGURES / "figure_manifest_v2_4.json")
