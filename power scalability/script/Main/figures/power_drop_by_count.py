"""Supplementary voltage-drop means by N and physical module; report unchanged."""
...
from power_revision_figures import (
    ROOT, FINAL, STYLE, LOADS, COLORS, MARKERS, LABELS, MODULES,
    DARK_CHARCOAL, PUBLICATION_STYLE, GRID, canvas, panel_label,
    load_handles, legend, export, digest, figure_47_drop_cmap,
)

SOURCE = ROOT / "DATA/analysis/current/all_power_review_20260907"
OUTPUT = ROOT / "DATA/analysis/current/voltage_drop_by_N_20260907"
        ...
        for n in range(1, 5):
            for i, module in enumerate(MODULES):
                ...
                groups.append({
                    ...
                    "independent_repeats_per_exact_condition": 1,
                })
...
def semantic():
    return {
        ...
        "independence": "One observation per exact combination/load; shared modules and sources, not independent repeat trials.",
        ...
        "report": "Supplementary figures only; published main 2.3 and existing figures unchanged.",
    }
...
def heatmap(groups):
    ...
    fig.text(.15,.095,"n = combinations containing that module; one reading per condition.", fontsize=7.5)
    fig.text(.15,.053,"* Includes the separate M0 retest estimate; see source notes.", fontsize=7.5)
    ...
    for load in LOADS:
        for i, module in enumerate(MODULES):
            ...
            if n == 3 and module == "M3" and load == "MAX":
                ax.annotate("44 mV", (x+jitter[-1], yy[-1]), xytext=(0,7),
...
def plot_four(branches, groups):
    ...
    fig.text(.105,.019,"* ZERO N=3 M0 includes a separate retest estimate; original fault is retained in source records.",fontsize=7)
...
def plot_single(n,branches,groups):
    ...
    if n == 3:
        fig.text(.12,.03,"* One ZERO M0 value uses the separate retest; the 44 mV loaded M3 point is retained.",fontsize=7.5)
...
def main():
    ...
    protected += list((FINAL/"Imperial College Individual Project Template_LaTeX/figures/power").glob("*"))
...
复现：从 Final 目录运行 python -B "power scalability/script/Main/figures/power_drop_by_count.py"。
...
"""
    ...
    print(json.dumps({"output":str(OUTPUT),"figures":len(figures),"summary_rows":len(groups),
                      "source_hashes_checked":source_count,"report_unchanged":True},indent=2))
