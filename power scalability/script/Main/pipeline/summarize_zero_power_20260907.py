...
MAX_SUPPS = {n: SUPPS / f"n{n}_max_20260907.csv" for n in (2, 3)}
MAX_METAS = {n: SUPPS / f"n{n}_max_20260907_metadata.json" for n in (2, 3)}
...
def build():
    sources = {VOLTAGES, EVENT, FROZEN_MANIFEST, ROOT / RELOCATION_REL, Path(__file__).with_name("photo_source_paths.py"), *MAX_SUPPS.values(), *MAX_METAS.values()}
    raw_cache = {}
    def raw(path):
        path = path.resolve()
        sources.add(path)
    ...
    # MAX supplements contribute coverage only, never ZERO measurements.
    max_supplement = defaultdict(dict)
    expected_max = {2: {"M0-M2", "M0-M3", "M1-M2", "M1-M3"}, 3: {"M0-M1-M3", "M0-M2-M3"}}
    for n, path in MAX_SUPPS.items():
        max_metadata = json.loads(MAX_METAS[n].read_text(encoding="utf-8"))
        assert max_metadata["load"] == "MAX", "MAX coverage metadata has another load"
        seen = set()
        for row in read_csv(path):
            combo, module = row["combo"], row["module_id"]
            ids = combo.split("-")
            assert row["load"] == "MAX" and len(ids) == n and ids == sorted(set(ids))
            assert set(ids) <= set(MODULES) and module in ids and module not in max_supplement[combo]
            ...
            temps = max_metadata["temperature_message_values"][combo]
            assert len(temps) == n + 2
            assert number(row["T_stm32_C"]) == number(temps[ids.index(module)])
            assert number(row["T_teensy_C"]) == number(temps[n])
            assert number(row["T_regulator_C"]) == number(temps[n + 1])
    ...
    for n in range(1, 5):
        ...
        for path in (BASE / f"n={n}").rglob("*.yaml"):
            ...
            if data.get("load") == "MAX":
                ...
                assert data["module_count"] == n and len(ids) == n and ids == sorted(set(ids))
                assert set(ids) <= set(MODULES)
                ...
                assert all(number(data.get(field)) is not None for field in required), path
                max_combos.append("-".join(ids))
        assert len(set(max_combos)) == len(max_combos)
        additions = {combo for combo in max_supplement if len(combo.split("-")) == n}
        assert not set(max_combos) & additions, "Duplicate MAX condition needs an explicit selection policy"
        max_combos.extend(additions)
        assert set(max_combos) == {c["combo"] for c in configs}
    ...
    assert sum(c["MAX_current_configurations"] for c in coverage) == 15
    summary = {"scope": "ZERO FULL configured 200 Hz continuous USB read; all 15 configurations of the existing four modules",
               ...
               "MAX_current_configuration_coverage": "15/15", "current_records_per_configuration_condition": 1,
               "MAX_coverage_basis": "Nine original MAX YAML configurations plus four N=2 and two N=3 MAX supplement configurations; every physical branch has V/I/T fields. Coverage only; all-load statistics are in DATA/analysis/current/all_power_review_20260907.",
...
def write_readme(configs, branches, groups, summary):
    ...
    lines += ["- 个别历史记录的主读数略超显示MIN/MAX范围，按原值保留并在summary.json标记；不静默修正。",
              ...
              "- MAX现覆盖15/15组合：原9组YAML、新增4组N=2和2组N=3 MAX均已记录各物理支路电压、电流与温度。这里仅更新覆盖度，不在ZERO统计中混入MAX数值；同条件独立重复仍未增加。",
              "- 全部ZERO/MAX组合及其描述性比较，见[完整Power统计总结](../all_power_review_20260907/README_CN.md)。",
              "- 最新N=2加载电压、电流、温度及与空载的对照，见[N=2满负载统计说明](../n2_max_review_20260907/README_CN.md)。", "",
              ...
              "- Power目录执行：`python script/Main/pipeline/summarize_zero_power_20260907.py`。本脚本只写本汇总目录，不修改原始记录或报告。", ""]
