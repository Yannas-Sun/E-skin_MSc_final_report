...
def build():
    sources = {SUPP, META, ROOT / RELOCATION_REL, Path(__file__).with_name("photo_source_paths.py")}
    ...
    for pair in itertools.combinations(("M0", "M1", "M2", "M3"), 2):
        ...
        for evidence in raw.get("voltage_photo_evidence", []):
            photo_path = resolve_photo(ROOT, path.parent / evidence["file"], evidence["sha256"])
        ...
        for mid in pair:
            ...
            photo = resolve_photo(ROOT, original_photo).relative_to(ROOT).as_posix() if original_photo else ""
            if photo:
...
def write_readme(configs, summary):
    ...
    lines += [f"- 合计电流CV：{a['I_sum_mA']['CV_percent']:.3f}%。", "",
              ...
              "- 本视图统计N=2 ZERO电流主读数、温度和配置功率估计，不混入MAX数值。最新加载结果见[满负载统计说明](../n2_max_review_20260907/README_CN.md)；同条件独立重复仍未增加。",
              ...
              "- 从Power目录执行：`python script/Main/pipeline/summarize_n2_zero_20260907.py`。", ""]
