...
def build():
    ...
    sources = {SUPP, META, ZERO, ZERO_ALL, FROZEN_MANIFEST}
    supplements = defaultdict(dict)
    for row in read_csv(SUPP):
        assert row["module_id"] not in supplements[row["combo"]]
        assert row["load"] == "MAX"
        supplements[row["combo"]][row["module_id"]] = row
    ...
    for n in range(1, 5):
        for group in itertools.combinations(("M0", "M1", "M2", "M3"), n):
            ...
            coverage.append({"module_count": n, "combo": combo, "ZERO_voltage_current_temperature_recorded": combo in zero_all,
                             ...
                             "MAX_load_mass_status": "unspecified_new_supplement" if combo in supplements else ("2000_g_recorded_area_distribution_unspecified" if combo in max_all else "missing"),
    ...
    summary = {"scope": metadata["scope"], "n_combinations": 6, "n_records_per_configuration_condition": 1,
               ...
               "overall_coverage": {"ZERO": "15/15", "MAX": "13/15", "MAX_missing": [r["combo"] for r in coverage if not r["MAX_voltage_current_temperature_recorded"]]},
    ...
    assert summary["overall_coverage"]["MAX_missing"] == ["M0-M1-M3", "M0-M2-M3"]
...
def write_readme(configs, pairs, summary):
    lines = ["# N=2 满负载电压、电流与温度补充统计（2026-09-07）", "",
             "新增四组按用户确认记为满负载 MAX。与原有 M0-M1、M2-M3 合并后，N=2 的 ZERO 和 MAX 均覆盖全部 6 种组合。整体 N=1–4 的 ZERO 已有 15/15；MAX 现有 13/15，仅缺 M0-M1-M3、M0-M2-M3。覆盖完成表示这些测点已有记录，不表示全部试验条件或故障监测已完整。", "",
    ...
    lines += ["", "上述 SD 是 6 种组合之间的样本 SD（n−1），不是同条件重复误差。每组合每负载条件 n_record = 1，无独立重复 SD；DMM 的 MIN/MAX 也不是 SD。各组合共享模块，不能当作 6 组独立硬件重复。", "",
              ...
              "- 从 Power 目录运行 `python script/Main/summarize_n2_max_20260907.py`。",
...
def main():
    ...
    protected += [REPORT, ROOT / "DATA/raw/canonical/power_raw.csv", SUPP, META]
