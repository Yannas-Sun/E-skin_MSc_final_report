...
def build():
    sources = {VOLTAGES, EVENT, FROZEN_MANIFEST}
    raw_cache = {}
    def raw(path):
        path = path.resolve()
        sources.add(path)
    ...
    for n in (2, 3):
        ...
        for row in read_csv(path):
            ...
            sources.add((ROOT / row["current_photo"]).resolve())
    ...
    assert sum(c["MAX_current_configurations"] for c in coverage) == 9
    summary = {"scope": "ZERO FULL configured 200 Hz continuous USB read; all 15 configurations of the existing four modules",
               ...
               "MAX_current_configuration_coverage": "9/15", "current_records_per_configuration_condition": 1,
...
def write_readme(configs, branches, groups, summary):
    ...
    lines += ["- 个别历史记录的主读数略超显示MIN/MAX范围，按原值保留并在summary.json标记；不静默修正。",
              ...
              "- MAX仍覆盖9/15组合：缺4个新增N=2及2个新增N=3组合；本次补充未增加MAX或同条件独立重复。", "",
...
def main():
    ...
    print(json.dumps({"configurations": len(configs), "branches": len(branches),
                      "MIN_MAX_coverage": summary["current_MIN_MAX_coverage"], "MAX_coverage": "9/15",
