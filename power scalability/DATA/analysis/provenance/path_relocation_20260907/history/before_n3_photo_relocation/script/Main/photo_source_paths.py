...
def resolve_photo(root: Path, reference: str | Path, expected_sha256: str | None = None) -> Path:
    ...
    if original.is_file():
        ...
        if original.parent.name not in ("M0-M2", "M0-M3", "M1-M2", "M1-M3") or original.parent.parent.name != "n=2":
            raise ValueError("Relocation is outside the audited N=2 ZERO configurations")
    else:
