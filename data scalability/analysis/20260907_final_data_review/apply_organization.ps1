...
if ($dataMovePlan.total_runs -ne 15 -or $dataMovePlan.total_files -ne 140) { throw 'Unexpected snapshot size' }
...
Write-Output "Organized $($dataMovePlan.total_runs) completed runs; verified $($dataVerified.Count) existing files, $($dataMovePlan.total_bytes) bytes."
