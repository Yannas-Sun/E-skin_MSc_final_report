"""Prepare a reversible, hash-checked plan for the 12 completed N=3 FULL captures.
...
RUN_IDS = '''20260907_000133_670681 20260907_000213_737553 20260907_000253_764287
...
20260907_003258_918645 20260907_003339_025463 20260907_003419_073823'''.split()
...
def main():
    ...
    for run_id in RUN_IDS:
        ...
        assert meta['target_hz'] == 200 and meta['N'] == 3 and len(meta['target_modules']) == 3
        assert window['status'] in ('PASS', 'CHECK REQUIRED')
        ...
        target_rel = f'FULL/N=3/200Hz/{module}/Repeat_{repeat}'
        ...
        row = dict(run_id=run_id, started_at=summary['started_at'],
                   mode='FULL', condition='zero_load', N=3, target_hz=200,
    ...
    assert set(groups) == {'M0_M1_M2','M0_M1_M3','M0_M2_M3','M1_M2_M3'}
    assert len({row['batch_id'] for row in index}) == 4
    stats, diagnostics = [], []
    for module, rows in sorted(groups.items()):
        ...
        for selected, output, basis in [(eligible, stats, 'PASS main windows only; integrity filter, no frame-rate filter'),
            ...
            item = dict(modules=module, N=3, target_hz=200, condition='zero_load',
    ...
    plan = dict(prepared_at=datetime.now().astimezone().isoformat(),
                ...
                rule='FULL/N=3/200Hz/M<slot>_M<slot>_M<slot>/Repeat_<within-batch-repeat>',
