...
SNAPSHOT = ("20260907_060147_020562","20260907_060227_053987","20260907_060307_083182")
...
def whole_packet_context(raw,row):
    offset,length = int(row["raw_offset"]),int(row["received_bytes"])
    return base.decode(raw[offset:offset+length]) | {"raw_offset":offset,"elapsed_s":float(row["elapsed_s"]),
             "packet_completed_time":row["packet_completed_time"],"main_window":row["main_window"]=="True"}
...
def add_context(directory,run,previous_run):
    ...
    combinations = {name:Counter() for name in ("full_run","initial","main")}
    missing_packets = []
    for i,row in enumerate(rows):
        offset,length = int(row["raw_offset"]),int(row["received_bytes"])
        decoded = base.decode(raw[offset:offset+length])
        if not decoded["outer_crc_valid"] or decoded["errors"]: continue
        by_slot = {m["slot"]:m for m in decoded["modules"]}
        key = "".join({"ESKD":"D","ESKF":"F"}.get(by_slot[s].get("marker"),"X")
                      if by_slot[s]["valid"] else "X" for s in run["metadata"]["target_modules"])
        combinations["full_run"][key] += 1
        combinations["main" if float(row["elapsed_s"])>=10 else "initial"][key] += 1
        missing = [s for s in run["metadata"]["target_modules"] if not by_slot[s]["valid"]]
        if missing:
            missing_packets.append({"missing_slots":missing,"context":{name:whole_packet_context(raw,rows[j])
                for name,j in (("before",i-1),("at",i),("after",i+1)) if 0<=j<len(rows)}})
    run["packet_frame_type_slot_order"] = run["metadata"]["target_modules"]
    run["packet_frame_type_combinations"] = {name:dict(c) for name,c in combinations.items()}
    run["missing_update_packets"] = missing_packets
    ...
    for event in run["inner_sequence_discontinuities"]:
        ...
        points = {"at":packet_context(raw,rows[i],slot)}
        # The immediately previous MUL1 may explicitly contain no module update.
        # Resolve the previous *valid module frame* matching the sequence audit.
        for j in range(i-1,-1,-1):
            candidate = packet_context(raw,rows[j],slot)
            if candidate["module"] and candidate["module"]["valid"]:
                points["before"] = candidate; break
        if i:
            points["immediately_before"] = packet_context(raw,rows[i-1],slot)
        for j in range(i+1,len(rows)):
            candidate = packet_context(raw,rows[j],slot)
            if candidate["module"] and candidate["module"]["valid"]:
                points["after"] = candidate; break
        ...
        current_module = points["at"]["module"]
        if current_module["valid"] and current_module.get("marker") == "ESKF":
            preceding = points["at"]
            for j in range(i+1,len(rows)):
                candidate = packet_context(raw,rows[j],slot)
                module = candidate["module"]
                if not module or not module["valid"]: continue
                if module["marker"] == "ESKD":
                    event["post_event_reconstruction"] = {
                        "observed_full_baseline_at_event":True,"baseline_elapsed_s":event["elapsed_s"],
                        "first_following_ESKD":candidate,
                        "following_ESKD_base_matches_previous_valid_frame":module["base_sequence"] == preceding["module"]["sequence"],
                        "pc_seconds_full_to_first_following_ESKD":candidate["elapsed_s"]-points["at"]["elapsed_s"],
                        "bridge_ms_full_to_first_following_ESKD":(candidate["host_ms"]-points["at"]["host_ms"])&0xffffffff,
                        "observed_ESKD_rejected_until_this_full":0,
                        "limitation":"This valid ESKF supplies a baseline immediately; no observed ESKD was rejected. Unrecorded data and fault onset/reason are unknown; this is not a measured physical recovery latency."}
                    break
                preceding = candidate
    for event in run["outer_sequence_discontinuities"]:
        i = index[event["offset"]]
        event["context"] = {name:whole_packet_context(raw,rows[j])
                            for name,j in (("before",i-1),("at",i),("after",i+1)) if 0<=j<len(rows)}
        difference = (event["sequence"]-event["previous"])&0xffffffff
        event["unobserved_sequence_values"] = difference-1 if 1<difference<0x80000000 else None
        event["main_window"] = event["elapsed_s"]>=10
        if i:
            event["bridge_host_gap_ms"] = (int(rows[i]["host_ms"])-int(rows[i-1]["host_ms"]))&0xffffffff
    actual = {}
...
def main():
    ...
    for rid in SNAPSHOT:
        ...
        run["actual_condition"] = "large_area"
        run["configured_module_count"] = len(summary["metadata"]["target_modules"])
        run["maximum_slots"] = summary["metadata"].get("M",4)
        run["actual_description"] = "四模块同时全覆盖按下"
        run["actual_condition_source"] = "user_confirmed_all_four_modules_simultaneous_full_coverage_press; original metadata preserved; equal_force_not_assumed"
        ...
        for scope in ("full_run","initial","main"):
            ...
            scope_outer = sum(scope == "full_run" or (scope == "main") == e["main_window"]
                              for e in run["outer_sequence_discontinuities"])
            run["scopes"][scope]["outer_sequence_discontinuity_count"] = scope_outer
            c = run["scopes"][scope]["counts"]
            scope_bad = run["scopes"][scope]["inner_sequence_discontinuity_count"] or scope_outer or any(c.get(k,0) for k in
                ("missing_valid_expected_updates","unexpected_valid_updates","target_mode_mismatch","base_break_events_after_anchor","unapplied_after_anchor","outer_crc_errors"))
            run["scopes"][scope]["independent_protocol_findings"] = "CHECK REQUIRED" if scope_bad else "NO DETECTED PROTOCOL ANOMALY"
    ...
    output = {"schema_version":1,"audit_at":datetime.now().astimezone().isoformat(),"snapshot_run_ids":list(SNAPSHOT),
              ...
              "condition_note":"metadata records zero_load; user explicitly confirmed simultaneous full-coverage pressing of all four modules; original metadata preserved; equal force is not assumed",
              "totals":totals,"runs":runs}
    (HERE/"raw_audit.json").write_text(json.dumps(output,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    qualification = []
    for r in runs:
        c = r["scopes"]["main"]["counts"]
        reasons = []
        for name,key in (("main_inner_sequence_discontinuities","inner_sequence_discontinuity_count"),
                         ("main_outer_sequence_discontinuities","outer_sequence_discontinuity_count")):
            if r["scopes"]["main"][key]: reasons.append(name)
        for key in ("missing_valid_expected_updates","unexpected_valid_updates","target_mode_mismatch", "base_break_events_after_anchor", "unapplied_after_anchor", "outer_crc_errors"):
            if c.get(key,0): reasons.append("main_"+key)
        if r["cross_check_errors"]: reasons.append("raw_log_summary_crosscheck_mismatch")
        if any(e.get("elapsed_s",-1)>=10 for e in r["anomalies"]): reasons.append("main_raw_protocol_decode_anomaly")
        qualification.append({"run_id":r["run_id"],"original_gui_main_status":r["scopes"]["main"]["reported_status"],
            "independent_main_status":"CHECK REQUIRED" if reasons else "PASS","main_review_reasons":reasons,
            "main_inner_sequence_events":r["scopes"]["main"]["inner_sequence_discontinuity_count"],
            "main_outer_crc_errors":c.get("outer_crc_errors",0),
            "main_outer_missing_sequence_numbers":sum(e["unobserved_sequence_values"] or 0 for e in r["outer_sequence_discontinuities"] if e["main_window"]),
            "main_missing_valid_expected_updates":c.get("missing_valid_expected_updates",0),
            "main_cache_base_error_events":c.get("base_break_events_after_anchor",0),
            "main_observed_base_discontinuities":sum(e["elapsed_s"]>=10 for e in r["observed_base_discontinuities"]),
            "main_unapplied_ESKD_after_anchor":c.get("unapplied_after_anchor",0),
            "main_valid_outer_packets":c["valid_outer_packets"],"main_valid_module_frames":c["valid_module_frames"],
            "actual_condition":"large_area","actual_description":"四模块同时全覆盖按下",
            "batch_selection_note":"Evaluate the three-run batch together; a passing R1 does not turn this batch into three passing repetitions."})
    (HERE/"qualification.json").write_text(json.dumps({"schema_version":1,"raw_audit_sha256":hashlib.sha256((HERE/"raw_audit.json").read_bytes()).hexdigest(),
        "runs":qualification,"original_files_unchanged":not changed},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    issues = sum(len(r["inner_sequence_discontinuities"]) for r in runs)
    lines = ["# N=4 四模块同时全覆盖三条 DELTA 记录：独立原始协议审计", "",
    ...
    lines += ["", "每包四模块帧型由raw独立解码，JSON顺序为M0/M1/M2/M3，D=ESKD、F=ESKF、X=槽位无有效更新。下表按一包内有效ESKF个数统计，含X的包独立列出；不能将含任意ESKF的包比例当作模块内q。",
              "", "|Repeat|主窗0F|主窗1F|主窗2F|主窗3F|主窗4F|有槽位未更新|", "|---|---:|---:|---:|---:|---:|---:|"]
    for r in runs:
        c = r["packet_frame_type_combinations"]["main"]
        values = [sum(v for k,v in c.items() if "X" not in k and k.count("F")==n) for n in range(5)]
        values.append(sum(v for k,v in c.items() if "X" in k))
        lines.append("|"+str(r["metadata"]["repeat"])+"|"+"|".join(map(str,values))+"|")
    ...
    for r in runs:
        ...
        for e in r["outer_sequence_discontinuities"]:
            lines.append(f"  外层序号{e['previous']}→{e['sequence']}，未观测编号{e['unobserved_sequence_values']}个，elapsed={e['elapsed_s']:.7f}s、raw_offset={e['offset']}、桥host_ms差={e.get('bridge_host_gap_ms')}ms。这是记录编号缺口，不能直接等同物理扫描丢失数。")
        for e in r["missing_update_packets"]:
            at = e["context"]["at"]
            statuses = {m['slot']:"0x{:02x}".format(m['status']) for m in at['modules']}
            lines.append(f"  有效MUL1 seq={at['sequence']}，elapsed={at['elapsed_s']:.7f}s、raw_offset={at['raw_offset']}，目标槽位{e['missing_slots']}未有效更新，status={statuses}。该包保留，不从计数中删除。")
