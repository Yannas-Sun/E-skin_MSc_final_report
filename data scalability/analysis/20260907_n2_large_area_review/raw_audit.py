...
SNAPSHOT = ("20260907_054752_155421","20260907_054832_183609","20260907_054912_204552")
...
def main():
    ...
    for rid in SNAPSHOT:
        ...
        run["actual_description"] = "大面积覆盖"
        run["actual_condition_source"] = "user_confirmed_large_area_coverage; original metadata preserved; loading equality across modules not assumed"
    ...
    output = {"schema_version":1,"audit_at":datetime.now().astimezone().isoformat(),"snapshot_run_ids":list(SNAPSHOT),
              ...
              "condition_note":"metadata records zero_load; user explicitly confirmed large-area coverage; original metadata preserved; identical loading across modules is not assumed",
    ...
    lines = ["# N=2 大面积覆盖三条 DELTA 记录：独立原始协议审计", "",
    ...
    lines += ["", "每包两模块帧型由raw独立解码。下表顺序为M1/M3，D=ESKD、F=ESKF，计数单位为外层MUL1包；不能将含任意ESKF的包比例当作模块内q。",
              "", "|Repeat|主窗DD|主窗DF|主窗FD|主窗FF|", "|---|---:|---:|---:|---:|"]
    for r in runs:
        c = r["packet_frame_type_combinations"]["main"]
        lines.append("|"+str(r["metadata"]["repeat"])+"|"+"|".join(str(c.get(k,0)) for k in ("DD","DF","FD","FF"))+"|")
