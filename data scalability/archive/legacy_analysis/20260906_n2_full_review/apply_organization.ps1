...
$dataAllowedRoot = [IO.Path]::GetFullPath('D:\study\programming\ESKIN\E-SKIN\docs\Final\data scalability\data')
if ([IO.Path]::GetFullPath($dataMovePlan.data_root) -ne $dataAllowedRoot) { throw 'Unexpected data root' }
if ($dataMovePlan.total_runs -ne 18 -or $dataMovePlan.total_files -ne 108) { throw 'Unexpected snapshot size' }
