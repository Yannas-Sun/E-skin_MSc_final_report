...
## 2026-09-07 - Report 2.6：图表取舍与3D＋2D理论模型

- Fig.4.2明确竖线为跨记录窗口的样本SD，B图为先平均窗口内各模块K，再汇总窗口；补全A/B各自n。
- 按用户要求移除N4异常专图，从动态比较图取消早期失败M1批次；原异常文字、六批动态表及源记录保留，性能统计未改。
- 原Fig.4.6改为1.0风格的USB/HOST SPI各3D曲面＋200Hz二维截面，现编号Fig.4.5。使用当前帧长度、USB 24+4M及一秒周期ESKF连续近似，随f调整q；10MHz测试报告与40MHz未测参考条件保持。
- 2.5版本完整保留，新PDF为main 2.6.pdf。原666文件哈希复核不变，原清单22/60不变；本次只调整图表呈现与解释。

## 2026-09-07 - Report 2.5：新固件 Data 结果纳入论文

- 使用现有111条记录改写报告Data方法、结果、讨论、协议/数据附录、摘要及结论；90条零载主矩阵覆盖FULL/DELTA全部15组合，每组合3个连续时间窗，另列12条完整通过的动态记录。
- 重绘6幅图，分开包长、实际包率和USB流量；报告mean ± sample SD与n，明确组合差异及连续时间重复的限制。全部ESKF均计入，原因字段未知；逐模块更新覆盖纳入平均模型，111条主窗字节账整数残差均为0 B。
- 保留6条CHECK记录，N4全覆盖整批列为诊断；每段4个外层缺失编号及7次模块前跳、R2空更新包均写入。CRC/base匹配不等价于无历史缺失，不归因于时钟或ESKF本身。
- 10 MHz保留测试报告/本地设置的证据边界，40 MHz仍为理论；容量包含q=0.005参考ESKF成本。K=480为逐模块等长交点，固定K外推仍为条件模型。
- 未改原始采集文件及历史固件数据，666份原文件和452份图表来源哈希复核通过。旧2.4源码/PDF已备份；新PDF为根目录main 2.5.pdf。同一Word/Markdown清单更新到22/60，详细版本归档见report_versions/v2.5/manifest.json。
